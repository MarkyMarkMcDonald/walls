...
const IndexPage = () => {
  ...
  let [puzzleData, setPuzzleData] = useLocalStorage(`github/MarkyMarkMcdonald/walls/${puzzleId}`, initialPuzzles.get(puzzleId))
...
const Stepper = ({ puzzleId, setPuzzleId, maxPuzzleId, minPuzzleId }) => {
  return (
    <StepperContainer>
      <Switcher disabled={puzzleId <= minPuzzleId} onClick={() => setPuzzleId(puzzleId - 1)}>{'<'}</Switcher>
      {puzzleId}
      <Switcher disabled={puzzleId >= maxPuzzleId} onClick={() => setPuzzleId(puzzleId + 1)}>{'>'}</Switcher>
    </StepperContainer>
  )
}
